...
class CNNBusinessHeadlines(scrapy.Spider):
    ...
    def __init__(self, output_dir, sections = None, retry = False, *args, **kwargs, ):
        ...
        #Only take today headlines
        # Set start date to yesterday at 23:59:59
        ...
        try:
            ...
            self.df = pd.DataFrame([], columns = ['title', 'date', 'text', 'authors', 'source', 'url', 'image_url', 'search_term'])
            print(str(e))
    ...
    def _process_html(self, html):
        ...
        for link in links:
            try:
                ...
                if data['date'] != None \
                and data['date'].month == self.start_date.month \
                and data['date'].day == self.start_date.day \
                and data['date'].year == self.start_date.year :
                    data_list.append([
                        data['title'],
                    ...
                    print(f"[WARNING] Skipping news because it was before today, title: {data['title']}")

            except Exception as e:
                print(f"[ERROR] Failed to fetch or process data at link {link}")
                print(str(e))
                continue
        
        new_df = pd.DataFrame(data_list, columns = ['title', 'date', 'text', 'authors', 'source', 'url', 'image_url', 'search_term'])
        #Concat with previous df
